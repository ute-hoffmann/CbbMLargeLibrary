...
def matrix_base_mpl(matrix, positions, substitutions, conservation=None,
    ...
    if max_value is None and min_value is None:
        # color map should be symmetric around value 1.0
        max_v = matrix_masked.max()
        print(max_v)
        min_v = matrix_masked.min()
        print(min_v)
        if (max_v - 1.0) > (abs(min_v)+1.0): 
            max_value = max_v
            min_value = 1.0 - (max_v-1)
        else:
            max_value = 1.0 + (abs(min_v)+1.0)
            min_value = min_v
...
#        min_value = -max_value
    ...
    colormap_conservation = mpl.cm.get_cmap(colormap_conservation).copy()
    ...
    if True:
        ...
        cb_2.ax.set_yticklabels(
            [
                "{symbol} {value:>+{width}.1f}".format(
                    symbol=s, value=v, width=0
                ) for (v, s) in [(0, symbol_min), (1, symbol_max)]
            ]
        )
        cb_2.ax.xaxis.set_ticks_position("none")
        cb_2.ax.yaxis.set_ticks_position("none")
        cb_2.outline.set_linewidth(0)
...
EVmut = pd.read_csv("../../../../input/EVcouplings_DeepSequ/TARGET_b0.3_single_mutant_matrix.csv")
df_values = pd.read_csv("single_muts_CLN2.csv")
df_values["sgRNA_target"]

positions_dict = {}
for i in df_values["sgRNA_target"]:
	number = int(i[1:-1])
	subst = i[-1]
	WT = i[:-1]
	if number not in positions_dict.keys():
		positions_dict[number] = [WT, [subst]]
	else:
		positions_dict[number][1].append(subst)
		
position_numbers = list(positions_dict.keys())
position_numbers.sort()

wmean_matrix = np.zeros(shape=((len(position_numbers),20)))
...
aa_dict = "HRKEDQNCTSGAVMLIYFWP" # sorted in same way as R. rubrum paper, according to biochemical properties
for i in range(len(position_numbers)):
	pos = position_numbers[i]
	if pos not in list(EVmut["pos"]):
		conservation_list.append(np.nan)
	else:
		conservation_list.append(list(EVmut[EVmut["pos"]==pos]["column_conservation"])[0])
	for j in range(len(aa_dict)):
		aa = aa_dict[j]
		if len(y_aa) < 20:
			y_aa.append(aa)
		if aa == positions_dict[pos][0][0]:
			wmean_matrix[i,j] = 1.0
			wildtype_seq.append(aa)
			x_ax_positions.append(str(pos))
		else:
			if len(df_values[df_values["sgRNA_target"]==positions_dict[pos][0] + aa]["norm"]) == 0:
				wmean_matrix[i,j] = np.nan
			else:
				wmean_matrix[i,j] = df_values[df_values["sgRNA_target"]==positions_dict[pos][0] + aa]["norm"]


# give 2.4 and -0.4 as max and min values so that all conditions have same scale

ax_object = matrix_base_mpl(wmean_matrix, x_ax_positions, y_aa, conservation=conservation_list,
                    secondary_structure=None, wildtype_sequence=wildtype_seq,
                    min_value=-0.4, max_value=2.4,
                    ax=None, colormap=plt.cm.RdBu_r,
                    colormap_conservation=plt.cm.Oranges, na_color="#bbbbbb",
                    title=None, position_label_size=8, substitution_label_size=8,
                    show_colorbar=True, colorbar_indicate_bounds=False,
                    show_wt_char=True, label_filter=None)
...
plt.savefig("subst_matrix_CLN2.png", dpi=300)
plt.savefig("subst_matrix_CLN2.pdf")
