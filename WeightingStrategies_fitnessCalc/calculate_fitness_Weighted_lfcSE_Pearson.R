...
DESeq_result_table <- dplyr::left_join(
  ...
  DESeq_result_table %>%
    ...
    dplyr::summarize(
      ...
      wmean_log2FoldChange = weighted.mean(log2FoldChange, sgRNA_correlation/(weight_lfcSE^2)),
      sd_log2FoldChange = sd(log2FoldChange),
      # gene fitness
      mean_fitness = weighted.mean(fitness, sgRNA_correlation/(weight_lfcSE^2)),
...
if (nrow(df_samplesheet) > 0) {
    message("Saving 'result_lfcSE_Pearson_Weighted.Rdata'")
    save(DESeq_result_table, file = "result_lfcSE_Pearson_Weighted.Rdata")
    message("Saving 'result_lfcSE_Pearson_Weighted.tsv'")
    readr::write_tsv(DESeq_result_table, file = "result_lfcSE_Pearson_Weighted.tsv")
}
