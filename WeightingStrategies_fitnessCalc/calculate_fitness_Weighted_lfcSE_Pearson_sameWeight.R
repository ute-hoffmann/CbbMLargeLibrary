...
#
# input arguments
setwd("~/Documents/bioinformatic_analyses/2024-05_LargeRubiscoLibrary/WeightingStrategies_fitnessCalc")
path_samplesheet <- "../input/samplesheet_1stCultiv_2ndCultiv.csv" # file paths to sample sheet
path_counts <- "../results/prepare/all_counts.tsv" # file paths to count tables
...
# Here, the major changes compared to the original script start: 
# correlation will be calculated, but neither correlation nor efficiency will 
# be taken into account for further averaging 
# adding on 11.11.2024: Somehow correlation is also not added into data frame...? But also not needed
...
# =======================================
#
# calculate gene fitness as mean of sgRNA fitness, no weights!
...
get_controls <- function(condition_spec) {
  if (gene_controls != "") {
    filter(df_controls, condition == condition_spec)$fitness
  } else {
    NULL
  }
}
df_WT <- DESeq_result_table %>% ungroup() %>%
  filter(str_detect(sgRNA_target, "WT"))
get_WT <- function(condition_spec) {
  df_WT[df_WT$condition==condition_spec & df_WT$time==0.0,]$fitness
}
...
DESeq_result_table <- dplyr::left_join(
  ...
  DESeq_result_table %>%
    ...
    dplyr::summarize(
      ...
      wmean_log2FoldChange = weighted.mean(log2FoldChange, sgRNA_correlation/weight_lfcSE),
      sd_log2FoldChange = sd(log2FoldChange),
      # gene fitness
      mean_fitness = weighted.mean(fitness, sgRNA_correlation/weight_lfcSE),
      sd_fitness = sd(fitness),
      # apply Wilcoxon rank sum test against Null hypothesis (fitness ~ 0) or
      # user-supplied set of controls
      p_fitness_Null = stats::wilcox.test(
        x = fitness,
        y = get_controls(condition),
        alternative = "two.sided"
      )$p.value,
      p_fitness_WT = stats::wilcox.test(
        x = fitness,
        y = get_WT(unique(condition)),
        alternative = "two.sided"
      )$p.value
    ),
  ...
  mutate(
    p_fitness_adj = stats::p.adjust(p_fitness_Null, method = "BH"),
    comb_score = abs(mean_fitness) * -log10(p_fitness_adj),
    p_fit_adj_WT = stats::p.adjust(p_fitness_WT, method="BH")
  )

df_norm <- data.frame(condition = unique(DESeq_result_table$condition), WT_fitness = NA, K214_fitness = NA)
for(cond in df_norm$condition){
  df_norm[df_norm$condition==cond,]$WT_fitness = unique(subset(DESeq_result_table, DESeq_result_table$sgRNA_target=="WT" & DESeq_result_table$condition == cond)$mean_fitness)
  df_norm[df_norm$condition==cond,]$K214_fitness = unique(median(subset(DESeq_result_table, grepl("K214", DESeq_result_table$sgRNA_target) & DESeq_result_table$condition == cond)$mean_fitness))
}

DESeq_result_table <- dplyr::left_join(DESeq_result_table, df_norm)
DESeq_result_table$norm <- (DESeq_result_table$mean_fitness-DESeq_result_table$K214_fitness)/(DESeq_result_table$WT_fitness-DESeq_result_table$K214_fitness)
...
if (nrow(df_samplesheet) > 0) {
    message("Saving 'result_lfcSE_Pearson_Weighted_sameWeight.Rdata'")
    save(DESeq_result_table, file = "result_lfcSE_Pearson_Weighted_sameWeight.Rdata")
    message("Saving 'result_lfcSE_Pearson_Weighted_sameWeight.tsv'")
    readr::write_tsv(DESeq_result_table, file = "result_lfcSE_Pearson_Weighted_sameWeight.tsv")
}
