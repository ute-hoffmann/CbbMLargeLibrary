...
get_WT <- function(condition) {
  filter(df_WT, condition == condition)$fitness
}
...
DESeq_result_table <- dplyr::left_join(
  ...
  DESeq_result_table %>%
    ...
    dplyr::summarize(
      .groups="keep",
      min_padj_pluspseudo = min(padj) + 7.587882e-286
    )
)
...
DESeq_result_table <- dplyr::left_join(
  ...
  DESeq_result_table %>%
    ...
    dplyr::summarize(
      ...
      # gene fitness
      mean_fitness = weighted.mean(fitness, -log10(min_padj_pluspseudo)),
...
if (nrow(df_samplesheet) > 0) {
    ...
    save(DESeq_result_table, file = "result_padj_Weighted.Rdata")
    message("Saving 'result_padj_Weighted.tsv'")
    readr::write_tsv(DESeq_result_table, file = "result_padj_Weighted.tsv")
}
