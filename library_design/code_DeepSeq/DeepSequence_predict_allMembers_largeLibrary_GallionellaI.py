...
output_dir = "results/2023-04-20_inSilicoEvolution_I/" # where do I find params?
...
def scoring_func(mutation_list):
    if mutation_list == "WT":
    	delta_elbos = data_helper.delta_elbo(vae_model, [], N_pred_iterations=N_ELBO_SAMPLES) 
    	return delta_elbos
...
for mut_variant in variants.keys():
	...
	print(str(n/len(variants.keys())*100) + "% done")
	
scored_variants["WT"] = [scoring_func("WT")]

dataframe = pd.DataFrame(scored_variants).transpose()
dataframe.to_csv("2024-07-30_variants_scored_DeepSequence.tsv", '\t', index=True)
