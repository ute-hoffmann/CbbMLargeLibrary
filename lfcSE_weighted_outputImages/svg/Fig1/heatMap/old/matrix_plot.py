...
df_values = pd.read_csv("single_muts.csv")
...
for i in range(len(position_numbers)):
	...
	for j in range(len(aa_dict)):
		...
		if aa == positions_dict[pos][0][0]:
			wmean_matrix[i,j] = 0.0
			wildtype_seq.append(aa)
			x_ax_positions.append(str(pos))
		else:
			if len(df_values[df_values["sgRNA_target"]==positions_dict[pos][0] + aa]["wmean_fitness"]) == 0:
				wmean_matrix[i,j] = np.nan
			else:
				wmean_matrix[i,j] = df_values[df_values["sgRNA_target"]==positions_dict[pos][0] + aa]["wmean_fitness"]
...
plt.savefig("subst_matrix.png", dpi=300)
plt.savefig("subst_matrix.pdf")
